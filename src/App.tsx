...
import { subscribeToSessions, syncSession, syncApiKeys, loadApiKeys } from "./lib/firebaseService";
...
export default function App() {
  ...
  const handleSaveApiKey = (key: string, model: string) => {
    ...
    if (user) {
      const newList = savedApiKeys.includes(key) ? savedApiKeys : [key, ...savedApiKeys];
      syncApiKeys(user.uid, newList);
    }
  };
  ...
  React.useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        // Load cloud API keys
        const cloudKeys = await loadApiKeys(currentUser.uid);
        if (cloudKeys.length > 0) {
          setSavedApiKeys(cloudKeys);
          localStorage.setItem(API_KEYS_LIST_STORAGE, JSON.stringify(cloudKeys));
        }

        // Subscribe to sessions
        const unsubscribeSessions = subscribeToSessions(currentUser.uid, (cloudSessions) => {
  ...
  // Sync session to cloud when changed
  React.useEffect(() => {
    if (user && currentSessionId) {
      const current = sessions.find(s => s.id === currentSessionId);
      if (current) {
        syncSession(user.uid, current);
      }
    }
  ...
  const handleDeleteSession = (id: string) => {
    ...
    if (user) {
      import("./lib/firebaseService").then(m => m.deleteSessionFromCloud(user.uid, id));
    }
  };
